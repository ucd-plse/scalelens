...
MAPPING_FILE=add-dn-file-snapdiff-$NUM_NODES.nm
FILE_COUNT=0
DN_COUNT=0
ITERATION=0
rm $MAPPING_FILE
...
for (( i = 1; i <= $NUM_NODES; i++ )); do
  ...
  ./nn-command.sh $PWD/test/NN-1 -mkdir $DDIR/d_$i
  ## each time we add more and more data
  for (( k = 0; k < $i; k++ )); do
    ./nn-command.sh $PWD/test/NN-1 -put $BLOCK_FILE $DDIR/d_$i/COPY_$k
  done
  ## we take a snapshot
  ./nn-command.sh $PWD/test/NN-1 -createSnapshot $DDIR snap-$i
  ## and then we diff, from the last to the first
  ./snap-diff.sh $PWD/test/NN-1 "$DDIR" "snap-1" "snap-$i"
  ## and record the current numbers
  DN_COUNT=$i
  ITERATION=$(($i - 1))
  FILE_COUNT=$(($FILE_COUNT + $i))
  echo "$ITERATION,$DN_COUNT,$FILE_COUNT" >> $MAPPING_FILE
  ## now, we wait...
...
sleep 20
## and collect results...
RESULTS="add-dn-file-snapdiff-$RANDOM"
