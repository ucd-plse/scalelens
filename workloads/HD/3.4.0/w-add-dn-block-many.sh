...
MAPPING_FILE=add-dn-block-many-$NUM_NODES.nm
TOKEN_COUNT=0
PEER_COUNT=0
ITERATION=0
rm $MAPPING_FILE
...
for (( i = 1; i <= $NUM_NODES; i++ )); do
  ...
  FACTOR=5000
  NUM_BLOCKS=$((i * FACTOR))  # Instead of 2^i, we use i^2 * FACTOR

  ## each time we add more and more data
  TMP_BLOCK_FILE=blk.txt
  echo -n "" > blk.txt  # commenting now to accumulate blocks over iterations 

  for (( k = 0; k < NUM_BLOCKS; k++ )); do
    cat $BLOCK_FILE >> $TMP_BLOCK_FILE
  done
  ## and then here add some blocks
   ./nn-command.sh $PWD/test/NN-1 -put $TMP_BLOCK_FILE /COPY_$i
  rm $TMP_BLOCK_FILE
...
sleep 20
## and collect results...
RESULTS="add-dn-block-many-$RANDOM"
