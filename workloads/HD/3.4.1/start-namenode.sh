...
## main class
MAIN="org.apache.hadoop.hdfs.server.namenode.NameNode"
...
## first lets format the stuff
CMD="$JAVA_CMD -cp \"$CLASSPATH\" $PROPERTIES $OP_HEAP $MAIN -conf $NODE_CONF/core-site.xml -conf $NODE_CONF/hdfs-site.xml -format -force"
echo "[$0] Executing [$CMD]"
$JAVA_CMD $PROPERTIES -cp "$CLASSPATH" $PROPERTIES $OP_HEAP $MAIN -conf $NODE_CONF/core-site.xml -conf $NODE_CONF/hdfs-site.xml -format -force
## and start it
...
CMD="$JAVA_CMD "$SL_AGENT_FLAGS" $PROPERTIES -cp "$CLASSPATH:$AGENT_JAR:$AGENT_ASSIST" $NN_HEAP $MAIN -conf $NODE_CONF/core-site.xml -conf $NODE_CONF/hdfs-site.xml &>$NODE_BASE/logs/system.log &"
echo "[$0] Executing [$CMD]"
...
$JAVA_CMD "$SL_AGENT_FLAGS" $PROPERTIES -cp "$CLASSPATH:$AGENT_JAR:$AGENT_ASSIST" $NN_HEAP $MAIN -conf $NODE_CONF/core-site.xml -conf $NODE_CONF/hdfs-site.xml &>$NODE_BASE/logs/system.log &
