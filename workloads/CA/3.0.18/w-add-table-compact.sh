...
sed -i 's/memtable_heap_space_in_mb: 2048/memtable_heap_space_in_mb: 4/' $CLUSTER_BASE/node-1/conf/cassandra.yaml
./start-node.sh $CLUSTER_BASE/node-1 $JMX_DEFAULT true
echo "[$0] Sleeping [$SLEEP_N] seconds before proceeding (wait for node start)"
sleep $SLEEP_N

## the mapping file
MAPPING_FILE=add-table-comp-$NUM_KS.nm
TABLE_COUNT=0
ITERATION=0
rm $MAPPING_FILE
...
for (( i = 1; i <= $NUM_KS; i++ )); do
  echo "[$0] Adding Table [$i] of [$NUM_KS]"
  ## go for a single table
  CMD="$PYTHON_CMD cql_utils.py 127.0.0.1 create_simple_table '{\"ks\":\"$KS\", \"table\":\"test_$i\", \"repfactor\":1, \"num_cols\":1, \"new_ks\":false, \"drop_ks\":false}'"
  eval "$CMD"
  ## alter the table to set the compaction strategy
  CMD="$PYTHON_CMD cql_utils.py 127.0.0.1 alter_table '{\"ks\":\"$KS\", \"tbl\":\"test_$i\" }'"
  echo $CMD
  ## and create a table, such that there is something to compact
  eval "$CMD"
  ## finally, put some data
  CMD="$PYTHON_CMD cql_utils.py 127.0.0.1 insert_rows_to '{\"next_id\":1, \"num_rows\":10, \"ks\":\"$KS\", \"tbl\":\"test_$i\"}'"
  eval "$CMD"
  sleep $SLEEP_B
  ## now, we measure
  printf "cmd=heapMeasure\nrunId=$i\ngcBefore=false\nenableLoopProfiling=true" >  $CLUSTER_BASE/node-1/$CMD_FILE
  ## and we snapshot, all keyspaces by default
  SNAP_CMD="./nodetool -h 127.0.0.1 -p $JMX_DEFAULT compact"
...
## and collect results...
RESULTS="add-table-compact-$RANDOM"
