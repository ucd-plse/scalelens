...
## the mapping file
MAPPING_FILE=add-ks-compact-$NUM_KS.nm
KS_COUNT=0
ITERATION=0
rm $MAPPING_FILE
...
for (( i = 1; i <= $NUM_KS; i++ )); do
  ...
  CMD="$PYTHON_CMD cql_utils.py 127.0.0.1 insert_rows_to '{\"next_id\":1, \"num_rows\":100, \"ks\":\"$KS\", \"tbl\":\"test\"}'"
  eval "$CMD"
  sleep $SLEEP_B
  ## now, we measure
  printf "cmd=heapMeasure\nrunId=$i\ngcBefore=false\nenableLoopProfiling=true" >  $CLUSTER_BASE/node-1/$CMD_FILE
  ## and we snapshot, all keyspaces by default
  COMP_CMD="./nodetool -h 127.0.0.1 -p $JMX_DEFAULT compact"
  echo "[$0] Executing [$COMP_CMD]"
  eval $COMP_CMD
  KS_COUNT=$i
  ITERATION=$(($i - 1))
