...
## the mapping file
MAPPING_FILE=add-row-scrub-$NUM_BATCHES.nm
ROW_COUNT=0
ITERATION=0
rm $MAPPING_FILE
...
for (( i = 1; i <= $NUM_BATCHES; i++ )); do
  ...
  sleep $SLEEP_B
  ## then we flush
  FLUSH_CMD="./nodetool -h 127.0.0.1 -p $JMX_DEFAULT flush scale test"
  echo "[$0] Executing [$FLUSH_CMD]"
  eval $FLUSH_CMD
  ## now, we measure
  printf "cmd=heapMeasure\nrunId=$i\ngcBefore=false\nenableLoopProfiling=true" >  $CLUSTER_BASE/node-1/$CMD_FILE
  ## and we scrub
  SCRUB_CMD="./nodetool -h 127.0.0.1 -p $JMX_DEFAULT scrub scale test"
  echo "[$0] Executing [$SCRUB_CMD]"
  eval $SCRUB_CMD
