...
MAPPING_FILE=add-dc-tok-decomm-$NUM_NODES.nm
TOKEN_COUNT=0
PEER_COUNT=0
ITERATION=0
rm $MAPPING_FILE
...
for (( i = 1; i <= $NUM_NODES; i++ )); do
  ...
  ## and proceed to decommision the nodes I started
  if [ "$i" -gt 1 ]; then
    for (( j = 1; j <= $NUM_DECOMS; j++ )); do
      PP2=$(($j + 1 + $JMX_BASE))
      ./nodetool $PP2 decommission -f
      echo "[$0] Sleeping [10] secs before decommisions is done..."
      sleep 10
    done
  fi
  ...
  ## and in order to reuse these nodes for later, I have to clear them out
  if [ "$i" -gt 1 ]; then
    for (( j = 1; j <= $NUM_DECOMS; j++ )); do
...
sleep 20
## and collect results...
RESULTS="add-dc-tok-decomm-$RANDOM"
