...
## Workload execution
MAPPING_FILE="add-bulk-data-$NUM_DATA_BATCH.nm"
DATA_BATCH_COUNT=0
ITERATION=0
...
for ((i=1; i<=$NUM_DATA_BATCH; i++)); do
  ...
  # compute number of records for this scale
  NUM_RECORDS=$(($i * 1024))
  echo "[$0] generating bulk insert with $NUM_RECORDS records"

  # prepare bulk insert SQL file
  BULK_SQL_FILE="bulk_insert_$i.sql"
  echo "INSERT INTO TestTable (id, col0, col1, col2, col3) VALUES" > "$BULK_SQL_FILE"

  for ((j=1; j<=$NUM_RECORDS; j++)); do
    IDX=$(($i * 1000000 + $j))
    if [ $j -eq $NUM_RECORDS ]; then
      echo "($IDX, 'col0-$j', $j, 'col2-$j', 'col3-$j');" >> "$BULK_SQL_FILE"
    else
      echo "($IDX, 'col0-$j', $j, 'col2-$j', 'col3-$j')," >> "$BULK_SQL_FILE"
    fi
  done
  ...
  bash sqlline.sh -u jdbc:ignite:thin://127.0.0.1/ -f "$BULK_SQL_FILE"
...
RESULTS="add-bulk-data-$(date +%Y%m%d%H%M%S)"
mkdir -p "$RESULTS"
