...
# ## Workload execution
MAPPING_FILE="add-cache-query-$NUM_DATA_BATCH.nm"
NUM_ENTRIES=0
ITERATION=0
...
#Insert into the Cache First

NUM_ENTRIES=10000

##Streaming is Faster than putAll
$JAVA_CMD -DIGNITE_HOME=$IGNITE_CLIENT_HOME -cp "$CLASSPATH" IgniteClient "$CLIENT_CONFIG_FILE" stream "$NUM_ENTRIES"
...
for ((i = 1; i <= $NUM_DATA_BATCH; i++)); do
  ...
  PAGE_SIZES=$(($PAGE_SIZES + 250))

  printf "cmd=heapMeasure\nrunId=$i\ngcBefore=false\nenableLoopProfiling=true" >"$CLUSTER_BASE/node-1/$CMD_FILE"

  echo "Running query operation with limit-$LIMIT and PAGE_SIZE-$PAGE_SIZES"

  $JAVA_CMD -DIGNITE_HOME=$IGNITE_CLIENT_HOME -cp "$CLASSPATH" IgniteClient "$CLIENT_CONFIG_FILE" textQuery $LIMIT $PAGE_SIZES value

  ITERATION=$(($i - 1))
  
  echo "$ITERATION,$PAGE_SIZES" >> $MAPPING_FILE
...
RESULTS="add-cache-query-$(date +%Y%m%d%H%M%S)"
mkdir -p "$RESULTS"
