...
if ! isInteger "$NUM_TABLES" || ! isInteger "$SLEEP_BETWEEN_STEPS" || ! isDirectory "$CLUSTER_BASE" || [ "$#" -ne 3 ]; then
  echo "[$0] USAGE: $0 <num_tables> <sleep_between_steps> <cluster_base>"
  echo "[$0] <num_tables>: Total number of tables to create in cluster"
...
RESULTS="add-table-$(date +%Y%m%d%H%M%S)"
mkdir -p "$RESULTS"
